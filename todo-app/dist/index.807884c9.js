...
_axiosDefault.default.get(apiUrl).then((response)=>{
    todos = response.data;
    render(list, todos);
});
// get ul with todos
...
function render(ul, todos1) {
    ...
    todos1.forEach((item)=>{
        ...
        buttonDel.addEventListener("click", (e)=>{
            e.preventDefault();
            _axiosDefault.default.delete(`${apiUrl}/${item.id}`).then(ul.removeChild(li));
        });
        div.append(h4_type);
        div.append(h4_date);
        content.append(div);
...
function openPage(updateUrl1) {
    window.open(updateUrl1, "_self");
}
function search() {
    const filter = document.querySelector(".search-bar").value.trim().toUpperCase();
    let filtered_todos = todos.filter((todo)=>{
        for(atr in todo){
            if (todo[atr].toUpperCase().includes(filter)) return todo;
        }
    });
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"JacNc"}]},["h1pKh","jBxsx"], "jBxsx", "parcelRequire66d6")
